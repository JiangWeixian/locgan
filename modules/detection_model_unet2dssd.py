...
class DetectModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netD = define_netD('res101', grah_netD['fm'], 3)
    ...
    def backward_D(self):
        ...
        '''
        # Fake RPN
        self.fake_target = self.netM.forward(self.resize_source)

        # Get FakeLoss
        # self.cgan_fake_input = torch.cat([self.fake_target, self.source], dim=0)
        self.pred_fake, _, _, _ = self.netD.forward(self.fake_target)
        self.loss_D_fake = self.criterionGAN(self.pred_fake, False)

        # Real
        # self.cgan_real_input = torch.cat([self.source, self.source], dim=0)
        self.pred_real, self.feats_real, self.real_conf, self.real_loc  = self.netD.forward(self.source)
        ...
        # Loss D
        self.loss_D = self.loss_D_fake + self.loss_D_real + (self.real_loss_l + self.real_loss_c)
        self.loss_loc = (self.real_loss_l + self.real_loss_c)
        # Backward D
    ...
    def backward_G(self):
        # Fake
        pred_fake, feats_fake, self.fake_conf, self.fake_loc = self.netD.forward(self.fake_target)
        ...
        # Loss D
        self.loss_G = self.loss_G_GAN +  (self.loss_fm + self.loss_l1) +(self.fake_loss_l + self.fake_loss_l)
    ...
    def current_errors(self):
        return {'loss_G': self.loss_G.data[0], 'loss_D': self.loss_D.data[0], 'loss_fake_loc': (self.fake_loss_c+self.fake_loss_l).data[0], 'loss_real_loc': (self.real_loss_c+self.real_loss_l).data[0]}
