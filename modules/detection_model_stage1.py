...
class DetectModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # init optimizer
        if opt.mode == 'train':
            self.G_solver = torch.optim.SGD(self.netG.parameters(), lr=opt.lr, momentum=opt.momentum, weight_decay=opt.weight_decay)
    ...
    def backward_G(self):
        ...
        # Loss D
        self.loss_G = (self.loss_G_GAN) * 0.1 +  (self.loss_l1 + self.loss_fm + self.fake_loss_l + self.fake_loss_c) * 0.9
    ...
    def save_location(self):
        output = self.detect(self.fake_loc, F.softmax(self.fake_conf.view(self.fake_conf.size(0), -1,  self.num_classes), dim=-1), self.priorboxes)
