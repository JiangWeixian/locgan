...
class TensorToAbsoluteCoords(object):
    def __call__(self, image, boxes=None, labels=None):
...
class TensorToPercentCoords(object):
    def __call__(self, image, boxes=None, labels=None):
        width, height = image.size()[1:]
        boxes[:, 0] /= float(width+1)
        boxes[:, 2] /= float(width+1)
...
class TensorNoise(object):
    ...
    def __call__(self, image, boxes, labels):
        ...
        for i in range(num):
            ...
            noise = torch.FloatTensor(1, 3, width, height).normal_(0, 1)
            image[:, ymin:ymax, xmin:xmax] = noise
        return image, boxes, labels  
...
class NoiseAugmentation(object):
    def __init__(self, size=300):
        self.size = size
        self.augment = Compose([
            TensorToAbsoluteCoords(),
            TensorNoise(),
            TensorToPercentCoords(),
        ])
